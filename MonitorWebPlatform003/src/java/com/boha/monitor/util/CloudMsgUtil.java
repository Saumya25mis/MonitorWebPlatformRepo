...
public class CloudMsgUtil {
    ...
    public ResponseDTO sendChatMessage(ChatMessageDTO msg) throws GCMException {
        ...
        try {
            ...
            cm.setChat(em.find(Chat.class, msg.getChatID()));
            cm.setCompanyStaff(em.find(CompanyStaff.class, msg.getCompanyStaffID()));
        ...
        for (GcmDevice de : gList) {
            for (ChatMember cm : cmList) {
                if (Objects.equals(cm.getCompanyStaff().getCompanyStaffID(), de.getCompanyStaff().getCompanyStaffID())) {
    ...
    public List<GcmDevice> sendRequestForTalk(ChatDTO ch, String msg, List<ChatMemberDTO> list) throws
        ...
        for (GcmDevice m : gList) {
            for (ChatMemberDTO cm : list) {
                if (Objects.equals(cm.getCompanyStaff().getCompanyStaffID(), m.getCompanyStaff().getCompanyStaffID())) {
        ...
        mm.setMessage(msg);
        mm.setDateSent(new Date());
        mm.setChatID(ch.getChatID());
        mm.setStaffName(ch.getCompanyStaff().getFirstName() + " " + ch.getCompanyStaff().getLastName());
        mm.setCompanyStaffID(ch.getCompanyStaff().getCompanyStaffID());
